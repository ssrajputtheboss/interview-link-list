                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        FutureBuilder(
                            ...
                            builder: (ctx,res){
                              if(res.hasData && res.connectionState == ConnectionState.done )
                        ...
                        //TextButton(onPressed: ()async{if(await canLaunch(_messageBody.link))launch(_messageBody.link);}, child: Text(_messageBody.link,style: TextStyle(color: Colors.blue),)),
...
class MessageWrapper extends StatelessWidget{
  ...
  Widget getEmojiByName(String name){
    ...
    return emoji == null ? Text(name,style: TextStyle( color: Colors.white,fontSize: 14),overflow: TextOverflow.clip,):emoji;
  }
  ...
  List<Widget> createWidgets(){
    ...
    widgets.add(Text(texts[0],style: TextStyle( color: Colors.white,fontSize: 14),overflow: TextOverflow.clip,));
    int i=1;
    re.allMatches(_msg).forEach((element) {
      widgets.add(getEmojiByName(element.group(0)!));
      widgets.add(Text(texts[i] , style: TextStyle( color: Colors.white,fontSize: 14),overflow: TextOverflow.clip,));
      ++i;
    });
    return widgets;
  }
...
class MessageBody{
  ...
  MessageBody(String msg,String time,String link,Map<String,int> reactions) : _msg= msg, _time = time,_link=link,_reactions=reactions;
  @override
  String toString() =>
    'msg:$_msg,time:$_time,link$_link,${_reactions.toString()}';
}
